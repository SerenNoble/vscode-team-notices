...
VSCode/WebIDE 插件：活动栏图标为「AI」，侧边栏包含五个功能入口：

| 入口 | 说明 | 内容来源 |
| --- | --- | --- |
| 🧩 Skill 介绍 | 团队技能手册（左树右详情导航页，页内可搜索） | 远端优先，内置兜底 |
| 🤖 模型选择和介绍 | 模型选择指南与可用模型说明 | 远端优先（JS 数据），内置兜底 |
| ❓ 帮助手册 | 常见问题（FAQ）与解决办法，页顶搜索框过滤 + 回车跳转 | 远端优先，内置兜底 |
| 📢 通知公告 | 版本更新 / 维护计划，展示最新 5 篇 | 远端，内置兜底 |
| ❌ 问题反馈 | 故障 / 咨询 / 需求 / 建议，提交到 Jenkins | — |

侧边栏图标使用 VSCode 原生 codicon（同尺寸严格对齐、跟随主题色）。
...
1. **远端**：GitHub 仓库最新内容
2. **缓存**：上次成功拉取的内容存在 VSCode globalState（跟用户配置走），断网时显示
3. **内置**：打包时固化在插件 `docs/` 与 `content.js` 里的快照，全新安装且从未联网时显示

因此离线环境打开插件仍有内容（可能是旧快照），这是有意设计——离线可用优先于内容新鲜度。
Skill 页回退直接到内置完整手册；帮助手册页底部会标注「远端拉取失败」当且仅当连缓存都没有。

### 帮助手册的搜索与跳转（页内交互）

- Webview 没有 `Ctrl+F` 查找控件，帮助手册页顶内置**关键字过滤框**：输入即隐藏未命中的问答块
- 过滤后按**回车**：平滑滚动到第一个命中章节并闪烁高亮（纯 JS 滚动；
  Webview 会拦截 `href="#id"` 原生锚点跳转，页内跳转必须用 JS 实现）
- 页面交互依赖 Webview 脚本：不支持脚本的定制版桌面上，搜索不生效但内容照常可读
...
| 帮助手册补充 FAQ | 改 `help.md`（自由 markdown），push |
